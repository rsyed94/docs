...
`passport discover` is the cross-platform headless scanner for Passport Discover. It reports safe configuration metadata only—never paths, commands or arguments, environment values, headers, credentials, endpoint paths or query strings, prompts, files, tool payloads, or traffic. See [Discover direct MCP connections](/admin/discover).
