...
For company accounts and durable one-action approvals, set up an identity in **Agents** and use the [managed access CLI and SDK](/admin/agents-and-access). Its credentials rotate independently of the agent.

For an existing legacy integration, create an agent identity and key in **Settings** (**Advanced → Agent keys (legacy)** on Free/Pro; the **Agent keys (legacy)** card on Enterprise), then inject both variables through your CI secret store. Environment-key mode wins over local profiles, writes nothing to disk, and works without `passport login`.
