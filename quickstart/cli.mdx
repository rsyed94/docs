...
<Steps>
  ...
  <Step title="Sign in once per workspace">
    ...
    Passport Cloud is the default when `--workspace` is provided. Self-hosted deployments also pass `--cloud <origin>`. Add `--no-open` when the browser must be opened manually.
...
The default profile uses `cli` as its policy client. Login activates the newly completed profile. When several profiles are stored, `passport profiles` lists them and `passport use <name>` changes the local active profile without contacting Passport. Resolution is explicit `--profile`, then `PASSPORT_PROFILE`, then the active profile, then the sole stored profile; scripts should use an explicit selector so they cannot accidentally run against another workspace.
