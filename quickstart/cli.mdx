...
<Steps>
  ...
  <Step title="Sign in once per machine">
    Copy the workspace id (`ws_…`) from your Passport browser URL or from the connector URL under **AI Clients → Add client** (`/w/ws_…/mcp`). Login opens a browser and stores the resulting refresh credential in the existing `~/.passport/credentials.json` profile store with owner-only permissions.
...
Agent keys are also non-interactive: they do not create action-time member approval prompts. Use a member profile for an operation that must pause for a human's one-off approval.
