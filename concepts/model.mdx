...
A bundle is an Enterprise virtual MCP: a named subset of tools drawn from across several MCPs, with its own endpoint (`/b/{bundleKey}/mcp`). Bundles let you hand an AI client a tight, curated tool set instead of everything a person can reach. See [Bundles](/admin/bundles) for how to create and connect one.
