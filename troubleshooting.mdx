...
Behavior depends on how the client attaches:

- **Bridge-backed local clients** (Claude Desktop, Cursor, VS Code, and similar clients wired through Passport Desktop or the local bridge): the bridge picks up an admin approval or a newly connected account and pushes it to running clients within about 15 seconds. Spec-compliant clients re-list automatically; if nothing shows after ~15 seconds, restart the client once. The app says as much when you connect: "will pick this up in ~15 seconds (restart if it doesn't appear)."
- **Hosted connectors** (ChatGPT, Claude.ai): reconnect or refresh the connector session in that product after an approval or account change; there is no local bridge poll.
- **Passport CLI**: run `passport status` to confirm the MCP is visible to the active profile, then `passport search` again. The CLI does not cache tool definitions between invocations. If an MCP is absent, check that the profile's policy client is allowed to see it under AI Clients in Passport.
- **Claude Code and Codex** on native MCP: start a new session so the client re-lists tools.
...
Some local AI clients cache their tool list at launch and do not react to a live bridge update. After you connect an account or an admin enables an MCP, if a bridge-backed client has not shown the tools within ~15 seconds, restart that client once. Hosted connectors and Passport CLI do not use this bridge refresh path.
